...
def test_export_user_transactions_excel_success(
    authenticated_client,
    test_user,
    test_bank,
):
    ...
    workbook = load_workbook(
        filename=BytesIO(response.data),
        read_only=True,
    )
    ...
    assert transaction_row[2] == 1000
    assert transaction_row[3] == "income"
    assert transaction_row[4] == "Salary"
    assert transaction_row[5] == "Income"
    assert transaction_row[6] == "Bank"
    assert transaction_row[7] == test_bank.name

def test_export_user_transactions_excel_with_no_history(
    authenticated_client,
    test_user,
):
    response = authenticated_client.get(
...
def test_export_user_transactions_excel_requires_authentication(
    client,
    test_user,
):
    response = client.get(
...
def test_export_user_transactions_excel_rejects_other_user(
    authenticated_client,
    other_user,
):
    response = authenticated_client.get(
...
def test_export_user_transactions_excel_rejects_nonexistent_user(
    authenticated_client,
):
    response = authenticated_client.get(
...
def test_export_user_transactions_pdf_success(
    authenticated_client,
    test_bank,
    test_user,
):
    response = authenticated_client.post(
        ...
        json={
            "amount": 500,
            "type": "income",
            "description": "Test PDF transaction",
            "category": "Test",
        },
    )
...
def test_export_user_transactions_pdf_with_no_history(
    authenticated_client,
    test_user,
):
    response = authenticated_client.get(
...
def test_export_user_transactions_pdf_requires_authentication(
    client,
    test_user,
):
    response = client.get(
...
def test_export_user_transactions_pdf_rejects_other_user(
    authenticated_client,
    other_user,
):
    response = authenticated_client.get(
...
def test_export_user_transactions_pdf_rejects_nonexistent_user(
    authenticated_client,
):
    response = authenticated_client.get(
